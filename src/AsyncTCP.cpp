...
AsyncClient::~AsyncClient() {
    ...
    PosixAsyncTCPManager::getInstance().unregisterClient(this);
}
...
bool AsyncClient::_connect_socket() {
    ...
    _pcb_busy    = true;
    _pcb_sent_at = millis();
    PosixAsyncTCPManager::getInstance().registerClient(this);
...
void AsyncClient::_handle_writable() {
    ...
    if (sent >= 0 && _sent_cb) {
        ...
        if (total > 0) {
            ...
            _sent_cb(_sent_cb_arg, this, total, elapsed);
            // Refresh timeout baseline when forward progress is made.
            _pcb_sent_at = millis();
        }
    } else {
        _pending_ack_len = 0;
...
void AsyncServer::_handle_accept() {
    ...
    AsyncClient* client = new (std::nothrow) AsyncClient();
    if (!client) {
        ::close(client_socket);
        return;
    }
...
void PosixAsyncTCPManager::registerClient(AsyncClient* client) {
    if (!client) {
        return;
    }
...
void PosixAsyncTCPManager::unregisterClient(AsyncClient* client) {
    if (!client) {
        return;
    }
    std::lock_guard<std::mutex> lock(_clients_lock);
...
void PosixAsyncTCPManager::_poll_thread_main() {
    while (_running) {
        ...
        {
            std::vector<AsyncClient*> clients_snapshot;
            {
                std::lock_guard<std::mutex> lock(_clients_lock);
                clients_snapshot = _clients;
            }

            for (auto client : clients_snapshot) {
                if (!client || !client->_registered || client->_socket < 0) {
                    continue;
                }

                int  socket_fd    = client->_socket;
                bool ready_read   = FD_ISSET(socket_fd, &readfds);
                bool ready_write  = FD_ISSET(socket_fd, &writefds);
                bool ready_except = FD_ISSET(socket_fd, &exceptfds);

                if (ready_read) {
                    client->_handle_readable();
                    // _handle_readable() can disconnect and delete client via callbacks.
                    // Never dereference client again in this iteration.
                    continue;
                }

                if (ready_write) {
                    client->_handle_writable();
                    // _handle_writable() can also trigger close/error callback chains.
                    continue;
                }
